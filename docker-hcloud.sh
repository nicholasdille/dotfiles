...
if test -f "${CONFIG_FILE}"; then
    ...
    hcloud server create --location fsn1 --image ubuntu-18.04 --name docker-${HCLOUD_DOCKER_VM_NAME} --ssh-key 116094 --type cx11 --user-data-from-file docker-user-data.txt
...
timeout 300 bash -c "while test -z '$(ssh root@${HCLOUD_DOCKER_VM_IP} ps -C dockerd --no-headers)'; do echo Waiting...; sleep 10; done"
if test -z "$?"; then
